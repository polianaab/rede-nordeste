...
import {
  ...
  Bell, Menu, X, ShieldCheck,
  AlertTriangle, CheckCircle, Package, ArrowRight, UserCheck,
  Newspaper, Image as ImageIcon, Plus, Edit2, Clock, Quote, ChefHat, Utensils,
  Trash2, ShoppingCart, Tag, XCircle
} from 'lucide-react';
import { getProdutosPendentes, aprovarOuRejeitarProduto } from '../../services/api';
...
export default function HomeAdmin() {
  ...
  const [confirmarExclusao, setConfirmarExclusao] = useState<{ aberto: boolean; id: number | null }>({ aberto: false, id: null });
  ...
  // ── NOVOS: produtos pendentes da API ─────────────────────────
  ...
  // ── Carrega pendentes quando aba verificacao é aberta ─────────
  useEffect(() => {
    if (abaAtiva === 'verificacao') {
      carregarPendentes();
    }
  }, [abaAtiva]);
  ...
  const carregarPendentes = async () => {
    ...
    try {
      ...
      setTotalPendentes(data.totalElements || 0);
    } catch {
  ...
  const handleStatusProduto = async (id: number, status: 'APROVADO' | 'REJEITADO') => {
    try {
      ...
      alert(status === 'APROVADO'
        ? '✅ Produto aprovado! Já aparece na vitrine.'
        : '❌ Produto rejeitado.');
    } catch (err: any) {
  ...
  // ── Funções de blog/receitas/destaques (sem alteração) ────────
  ...
  const handleEditarNoticia = (noticia: any) => {
    setFormNoticia({
      id: noticia.id, titulo: noticia.titulo, subtitulo: noticia.subtitulo || '',
      imagem: noticia.imagem || '', descricao: noticia.descricao || noticia.conteudo || '',
      citacao: noticia.citacao || '', tempo: noticia.tempoLeitura || noticia.tempo || '3 min',
  ...
  const salvarReceita = () => {
    ...
    let listaAtualizada;
    if (formReceita.id) {
      listaAtualizada = receitasGlobais.map(r => r.id === formReceita.id ? { ...formReceita, data: r.data } : r);
    } else {
    ...
    <div className="min-h-screen bg-[#F5F2ED] text-[#394158] font-sans flex flex-col md:flex-row overflow-x-hidden">
      ...
      <aside className={`fixed md:sticky top-0 left-0 h-screen bg-[#1a1f2e] text-white p-6 flex flex-col z-[100] transition-transform duration-300 w-72 shadow-2xl ${menuAberto ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <nav className="flex-1 space-y-1 overflow-y-auto no-scrollbar">
          {[
            ...
            <button key={item.id} onClick={() => { setAbaAtiva(item.id); setMenuAberto(false); }}
              className={`w-full flex items-center gap-4 px-5 py-4 rounded-[1rem] transition-all font-black uppercase text-[10px] tracking-widest ${abaAtiva === item.id ? 'bg-[#f9943b] text-white shadow-lg' : 'text-white/40 hover:bg-white/10'}`}>
              <item.icon size={18} /> {item.label}
              {/* Badge de pendentes na sidebar */}
              {item.id === 'verificacao' && totalPendentes > 0 && (
        ...
        <div className="mt-auto pt-6 border-t border-white/10">
          <button onClick={() => navigate('/home2')} className="w-full px-6 py-4 text-[#f9943b] font-black uppercase text-[10px] tracking-widest hover:bg-[#f9943b]/10 rounded-[1rem] transition-all flex items-center gap-3">
            <ArrowRight size={16} /> Sair do Painel
          </button>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0">
        ...
        <div className="p-6 md:p-12">
          ...
          {/* VERIFICAÇÃO — usuários mockados + produtos REAIS da API */}
          {abaAtiva === 'verificacao' && (
            <div className="animate-in slide-in-from-bottom-4 duration-500 space-y-8">

              {/* Solicitações de usuários (mockado) */}
              <div className="bg-white p-8 rounded-[1rem] border border-gray-100 shadow-sm">
                <h2 className="text-xl font-black uppercase italic tracking-tighter text-[#394158]">Aprovações Pendentes</h2>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1 mb-6">Verifique os documentos antes de liberar o vendedor</p>
              ...
              {/* Produtos pendentes — DADOS REAIS DA API */}
              <div className="bg-white p-8 rounded-[1rem] border border-gray-100 shadow-sm">
                <div className="flex items-center justify-between mb-2">
                  <h2 className="text-xl font-black uppercase italic tracking-tighter text-[#394158]">
                    Produtos Aguardando Aprovação
                    {totalPendentes > 0 && (
                      <span className="ml-3 bg-red-500 text-white text-[10px] font-black px-3 py-1 rounded-full">
                        {totalPendentes}
                      </span>
                    )}
                  </h2>
                  <button onClick={carregarPendentes}
                    className="text-[10px] font-black uppercase text-[#55833d] hover:underline transition-all">
                    ↻ Atualizar
                  </button>
                </div>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-6">
                  Produtos cadastrados pelos produtores — aprove para exibir na vitrine
                </p>
                ...
                {carregandoPendentes ? (
                  ...
                  <div className="space-y-4">
                    {produtosPendentes.map((prod: any) => (
                      <div key={prod.id}
                        className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-5 bg-gray-50 border border-gray-100 rounded-[1rem] hover:border-[#f9943b]/30 transition-all">
                        <div className="flex items-center gap-4">
                          <img
                            src={prod.imagemUrl || 'https://via.placeholder.com/64'}
                            className="w-16 h-16 rounded-xl object-cover border border-gray-100 shrink-0"
                            alt={prod.nome}
                          />
                          <div>
                            <h4 className="font-black text-sm uppercase text-[#394158]">{prod.nome}</h4>
                            <p className="text-[10px] font-bold text-gray-400 uppercase mt-1">
                              Loja: {prod.nomeLoja} · R$ {Number(prod.precoAtual).toFixed(2)}/{prod.unidadeMedida}
                            </p>
                            <p className="text-[10px] font-black text-[#f9943b] uppercase mt-1">
                              Categoria: {prod.nomeCategoria || '—'} · Estoque: {prod.estoqueAtual}
                            </p>
                            {prod.descricao && (
                              <p className="text-[9px] text-gray-400 mt-1 line-clamp-1 italic">{prod.descricao}</p>
                            )}
                          </div>
                        </div>
                        <div className="flex gap-2 w-full md:w-auto shrink-0">
                          <button onClick={() => handleStatusProduto(prod.id, 'REJEITADO')}
                            className="flex-1 md:flex-none px-6 py-3 bg-red-50 text-red-500 text-[9px] font-black uppercase tracking-widest rounded-xl hover:bg-red-100 transition-colors flex items-center justify-center gap-1">
                            <XCircle size={14} /> Rejeitar
                          </button>
                          <button onClick={() => handleStatusProduto(prod.id, 'APROVADO')}
                            className="flex-1 md:flex-none px-6 py-3 bg-[#55833d] text-white text-[9px] font-black uppercase tracking-widest rounded-xl hover:bg-[#436b2f] transition-colors flex items-center justify-center gap-1">
          ...
          {abaAtiva === 'destaques' && (
            <div className="animate-in slide-in-from-bottom-4 duration-500 space-y-8">
              <div className="flex flex-col sm:flex-row justify-between items-center bg-white p-8 rounded-[1rem] border border-gray-100 gap-4 shadow-sm">
                ...
                <button onClick={() => { setFormDestaque({ id: null, tipo: 'DESTAQUE', titulo: '', subtitulo: '', img: '', corDestaque: 'text-[#f9943b]', blogId: '' }); setModalDestaque(true); }}
                  className="w-full sm:w-auto bg-[#f9943b] text-white px-8 py-4 rounded-[1rem] font-black uppercase text-[10px] tracking-[0.2em] flex items-center justify-center gap-3 shadow-lg hover:bg-[#e08635] transition-all">
                ...
                {destaquesHome.map(destaque => (
                  <div key={destaque.id} className="bg-white rounded-[1.5rem] overflow-hidden shadow-xl border border-white flex flex-col group relative">
                    <div className="aspect-video relative overflow-hidden">
                      ...
                      <div className={`absolute top-4 left-4 bg-white/90 backdrop-blur px-3 py-1 rounded-full text-[8px] font-black uppercase shadow-sm ${destaque.corDestaque}`}>{destaque.tipo}</div>
                    </div>
          ...
          {abaAtiva === 'noticias' && (
            <div className="animate-in slide-in-from-bottom-4 duration-500 space-y-8">
              <div className="flex flex-col sm:flex-row justify-between items-center bg-white p-8 rounded-[1rem] border border-gray-100 gap-4 shadow-sm">
                ...
                <button onClick={() => { setFormNoticia({ id: null, titulo: '', subtitulo: '', imagem: '', descricao: '', citacao: '', tempo: '3 min', categoria: 'NOTÍCIA' }); setModalNoticia(true); }}
                  className="w-full sm:w-auto bg-[#55833d] text-white px-8 py-4 rounded-[1rem] font-black uppercase text-[10px] tracking-[0.2em] flex items-center justify-center gap-3 shadow-lg hover:bg-[#436b2f] transition-all">
                    ...
                    <div className="p-6 flex-1 flex flex-col">
                      ...
                      <div className="mt-auto pt-6 flex justify-between items-center border-t border-gray-50">
                        ...
                        <div className="flex items-center gap-1">
                          <button onClick={() => handleEditarNoticia(noticia)} className="p-2 text-gray-300 hover:text-[#f9943b] transition-colors cursor-pointer"><Edit2 size={16} /></button>
                          <button onClick={() => setConfirmarExclusao({ aberto: true, id: noticia.id })} className="p-2 text-gray-300 hover:text-red-500 transition-colors cursor-pointer"><Trash2 size={16} /></button>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          {abaAtiva === 'receitas' && (
            <div className="animate-in slide-in-from-bottom-4 duration-500 space-y-8">
              <div className="flex flex-col sm:flex-row justify-between items-center bg-white p-8 rounded-[1rem] border border-gray-100 gap-4 shadow-sm">
                ...
                <button onClick={() => { setFormReceita({ id: null, titulo: '', descricao: '', imagem: '', ingredientes: '', preparo: '', tempo: '40 min', porcoes: '4 porções', dificuldade: 'Fácil' }); setModalReceita(true); }}
                  className="w-full sm:w-auto bg-[#f9943b] text-white px-8 py-4 rounded-[1rem] font-black uppercase text-[10px] tracking-[0.2em] flex items-center justify-center gap-3 shadow-lg hover:bg-[#e88127] transition-all">
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-sm rounded-[1.5rem] p-8 shadow-2xl text-center animate-in zoom-in-95">
            ...
            <div className="flex gap-3">
              <button onClick={() => setConfirmarExclusao({ aberto: false, id: null })} className="flex-1 py-4 bg-gray-100 text-gray-400 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-gray-200 transition-all">Não, cancelar</button>
              <button onClick={() => confirmarExclusao.id && handleApagarNoticia(confirmarExclusao.id)} className="flex-1 py-4 bg-red-500 text-white text-[10px] font-black uppercase tracking-widest rounded-xl shadow-lg hover:bg-red-600 transition-all">Sim, apagar</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-2xl rounded-[1.5rem] overflow-hidden shadow-2xl animate-in zoom-in-95">
            <header className="p-6 border-b flex justify-between items-center bg-[#F5F2ED]">
              <h3 className="font-black uppercase italic tracking-widest text-[#394158]">Alimentar Blog</h3>
              <button onClick={() => { setFormNoticia({ id: null, titulo: '', subtitulo: '', imagem: '', descricao: '', citacao: '', tempo: '3 min', categoria: 'NOTÍCIA' }); setModalNoticia(false); }} className="p-2 hover:bg-white rounded-full"><X size={24} /></button>
            </header>
            <div className="p-8 space-y-6 max-h-[75vh] overflow-y-auto no-scrollbar">
              <div className="grid grid-cols-1 gap-5">
                <div className="space-y-1">
                  ...
                  <select value={formNoticia.categoria} onChange={e => setFormNoticia({ ...formNoticia, categoria: e.target.value })} className="w-full p-4 bg-[#F5F2ED]/50 text-[#394158] font-bold rounded-[1rem] outline-none border-2 border-transparent focus:border-[#f9943b] appearance-none">
      ...
      {modalReceita && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-2xl rounded-[1.5rem] overflow-hidden shadow-2xl animate-in zoom-in-95">
            <header className="p-6 border-b flex justify-between items-center bg-[#F5F2ED]">
              <h3 className="font-black uppercase italic tracking-widest text-[#394158]">Alimentar Receitas</h3>
              <button onClick={() => { setFormReceita({ id: null, titulo: '', descricao: '', imagem: '', ingredientes: '', preparo: '', tempo: '40 min', porcoes: '4 porções', dificuldade: 'Fácil' }); setModalReceita(false); }} className="p-2 hover:bg-white rounded-full"><X size={24} /></button>
            </header>
            <div className="p-8 space-y-6 max-h-[75vh] overflow-y-auto no-scrollbar">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="col-span-2 space-y-1"><label className="text-[10px] font-black uppercase text-[#f9943b] tracking-widest ml-2">Dificuldade</label><select value={formReceita.dificuldade} onChange={e => setFormReceita({ ...formReceita, dificuldade: e.target.value })} className="w-full p-4 bg-[#F5F2ED]/50 text-[#394158] font-medium rounded-[1rem] outline-none border-2 border-transparent focus:border-[#f9943b] appearance-none"><option value="Fácil">Fácil</option><option value="Média">Média</option><option value="Difícil">Difícil</option></select></div>
                <div className="col-span-2 space-y-1"><label className="text-[10px] font-black uppercase text-[#f9943b] tracking-widest ml-2">Foto do Prato Pronto</label><label className="w-full p-4 bg-[#F5F2ED]/50 text-gray-400 font-bold rounded-[1rem] border-2 border-dashed border-gray-200 hover:border-[#f9943b] flex items-center justify-center gap-2 cursor-pointer truncate"><ImageIcon size={18} /> {formReceita.imagem ? 'Selecionada ✓' : 'Escolher Foto'}<input type="file" className="hidden" onChange={handleReceitaImage} accept="image/*" /></label></div>
                ...
                <div className="col-span-2 space-y-1">
                  ...
                  <textarea rows={4} value={formReceita.ingredientes} onChange={e => setFormReceita({ ...formReceita, ingredientes: e.target.value })} placeholder={"500g de Feijão Verde\n300g de Queijo Coalho\n1 maço de Coentro"} className="w-full p-4 bg-[#55833d]/5 text-[#394158] font-medium rounded-[1rem] outline-none border-2 border-transparent focus:border-[#55833d] resize-none leading-loose" />
                </div>
                <div className="col-span-2 space-y-1"><label className="text-[10px] font-black uppercase text-[#394158] tracking-widest ml-2">Modo de Preparo</label><textarea rows={5} value={formReceita.preparo} onChange={e => setFormReceita({ ...formReceita, preparo: e.target.value })} placeholder={"Passo 1: Ferva a água...\nPasso 2: Misture..."} className="w-full p-4 bg-[#F5F2ED]/50 text-[#394158] font-medium rounded-[1rem] outline-none border-2 border-transparent focus:border-[#f9943b] resize-none" /></div>
              </div>
      ...
      {modalDestaque && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white w-full max-w-2xl rounded-[1.5rem] overflow-hidden shadow-2xl animate-in zoom-in-95">
            ...
            <div className="p-8 space-y-6 max-h-[75vh] overflow-y-auto no-scrollbar">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-1"><label className="text-[10px] font-black uppercase text-[#f9943b] tracking-widest ml-2">Tag do Banner</label><input type="text" value={formDestaque.tipo} onChange={e => setFormDestaque({ ...formDestaque, tipo: e.target.value })} placeholder="Ex: SAFRA DO MÊS" className="w-full p-4 bg-[#F5F2ED]/50 text-[#394158] font-bold rounded-[1rem] outline-none border-2 border-transparent focus:border-[#f9943b]" /></div>
                <div className="space-y-1"><label className="text-[10px] font-black uppercase text-[#f9943b] tracking-widest ml-2">Cor da Tag</label><select value={formDestaque.corDestaque} onChange={e => setFormDestaque({ ...formDestaque, corDestaque: e.target.value })} className="w-full p-4 bg-[#F5F2ED]/50 text-[#394158] font-bold rounded-[1rem] outline-none border-2 border-transparent focus:border-[#f9943b] appearance-none"><option value="text-[#f9943b]">Laranja</option><option value="text-[#55833d]">Verde</option><option value="text-[#C4D663]">Verde Limão</option><option value="text-blue-400">Azul</option><option value="text-red-400">Vermelho</option></select></div>
                <div className="col-span-2 space-y-1"><label className="text-[10px] font-black uppercase text-[#f9943b] tracking-widest ml-2">Título Grande</label><input type="text" value={formDestaque.titulo} onChange={e => setFormDestaque({ ...formDestaque, titulo: e.target.value })} placeholder="Ex: A melhor época para comprar manga" className="w-full p-4 bg-[#F5F2ED]/50 text-[#394158] font-black text-xl italic rounded-[1rem] outline-none border-2 border-transparent focus:border-[#f9943b]" /></div>
                <div className="col-span-2 space-y-1"><label className="text-[10px] font-black uppercase text-[#f9943b] tracking-widest ml-2">Subtítulo (Opcional)</label><input type="text" value={formDestaque.subtitulo} onChange={e => setFormDestaque({ ...formDestaque, subtitulo: e.target.value })} placeholder="Ex: Produtos frescos e com preços..." className="w-full p-4 bg-[#F5F2ED]/50 text-[#394158] font-medium rounded-[1rem] outline-none border-2 border-transparent focus:border-[#f9943b]" /></div>
                <div className="space-y-1"><label className="text-[10px] font-black uppercase text-[#f9943b] tracking-widest ml-2">Imagem de Fundo</label><label className="w-full p-4 bg-[#F5F2ED]/50 text-gray-400 font-bold rounded-[1rem] border-2 border-dashed border-gray-200 hover:border-[#f9943b] flex items-center justify-center gap-2 cursor-pointer truncate"><ImageIcon size={18} /> {formDestaque.img ? 'Selecionada ✓' : 'Escolher Foto'}<input type="file" className="hidden" onChange={handleDestaqueImage} accept="image/*" /></label></div>
                <div className="space-y-1"><label className="text-[10px] font-black uppercase text-[#f9943b] tracking-widest ml-2">Link p/ Notícia (Blog ID)</label><select value={formDestaque.blogId} onChange={e => setFormDestaque({ ...formDestaque, blogId: e.target.value })} className="w-full p-4 bg-[#F5F2ED]/50 text-[#394158] font-bold rounded-[1rem] outline-none border-2 border-transparent focus:border-[#f9943b] appearance-none"><option value="">Nenhuma notícia</option>{noticiasGlobais.map(n => (<option key={n.id} value={n.id}>{n.titulo}</option>))}</select></div>
              </div>
      ...
      <footer className="md:hidden bg-white border-t p-6 text-center"><span className="text-[8px] font-black uppercase text-gray-300">©️ 2026 Admin — Rede Nordeste</span></footer>
    </div>
  );
}